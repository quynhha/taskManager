...
public class GetTeammatesFromTaskHandler implements RequestHandler<GetTeammateFromTaskRequest, GetTeammatesFromTaskResponse> {
	...
	List<Teammate> GetTeammatesFromTask(String taskName, String projectName) throws Exception{
	...
	public GetTeammatesFromTaskResponse handleRequest(GetTeammateFromTaskRequest req, Context context)  {
		...
		try {
			...
			List<Teammate> teammates = GetTeammatesFromTask(req.taskName, req.projectName);

			
			response = new GetTeammatesFromTaskResponse(teammates,200);
		} catch (Exception e) {
